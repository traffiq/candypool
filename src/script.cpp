                  ...
                  CScript& scriptSigRet, txnouttype& whichTypeRet)
{
    ...
    case TX_PUBKEYHASH:
        keyID = CKeyID(uint16_t0(vSolutions[0]));
        if (!Sign1(keyID, keystore, hash, nHashType, scriptSigRet))
            return false;
        else
        {
        ...
        return true;
    case TX_SCRIPTHASH:
        return keystore.GetCScript(uint16_t0(vSolutions[0]), scriptSigRet);
...
bool IsMine(const CKeyStore &keystore, const CScript& scriptPubKey)
{
    ...
    {
        CScript subscript;
        if (!keystore.GetCScript(CScriptID(uint16_t0(vSolutions[0])), subscript))
            return false;
...
bool ExtractDestination(const CScript& scriptPubKey, CTxDestination& addressRet)
{
    ...
    {
        addressRet = CKeyID(uint16_t0(vSolutions[0]));
        return true;
    }
    else if (whichType == TX_SCRIPTHASH)
    {
        addressRet = CScriptID(uint16_t0(vSolutions[0]));
        return true;
    }
