...
void SetExtraNonce(CBlock* pblock, CBlockIndex* pindexPrev,
					unsigned int& nExtraNonce, bool bWithMerkle ) {

	pblock->nNonce = nExtraNonce;
	unsigned int nHeight = pindexPrev->nHeight + 1; // Height first in coinbase required for block.version=2
	const char* pszDedication =
			mapArgs.count("-dedication") ? mapArgs["-dedication"].c_str() : "";
	pblock->vtx[0].vin[0].scriptSig = (CScript() << nHeight
			<< CBigNum(nExtraNonce)
			<< vector<unsigned char>((const unsigned char*) pszDedication,
					(const unsigned char*) pszDedication
							+ strlen(pszDedication))) + COINBASE_FLAGS;
	assert(pblock->vtx[0].vin[0].scriptSig.size() <= 100);
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet) {
	...
	fprintf(stderr , "Current Pool Wallet Adress: <%s>\n", CBitcoinAddress(pwallet->vchDefaultKey.GetID()).ToString().c_str());

	// settings from config file
	if (mapArgs["-sqlhost"] == "" || mapArgs["-sqluser"] == "" || mapArgs["-poolport"] == "")
	{
		fprintf(stderr , "Error: SQL configuration or poolport is not set.\n");
		return;
	}
	...
	// set all miners to offline and connect to db
	try
	{
		mysqlpp::Connection *conn = new mysqlpp::Connection();
		if (conn->connect(strDb.c_str(), strHost.c_str(), strUser.c_str(), strPw.c_str())) {
		...
		query << "UPDATE "<< mapArgs["-sqldatabase"] << ".`worker` SET `online` = '0'";
		query.execute();
		conn->disconnect();
	}
	...
	new boost::thread(xptServer_startProcessingEPoll, pwallet);

	// confirmation checker thread
	mysqlpp::Connection *connConfirm = new mysqlpp::Connection();
	if (connConfirm->connect(strDb.c_str(), strHost.c_str(), strUser.c_str(), strPw.c_str())) {
		new boost::thread(boost::bind(&xptServer_checkBlockConfirmations, connConfirm, pwallet));
	} else {
	   fprintf(stderr, "DB connection failed for confirmation thread! \n");
	   return;
	}
}
