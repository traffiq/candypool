...
uint32_t stream_copy(stream_t* dest, stream_t* source, uint32_t length)
{
	...
	while( length > 0 )
	{
		uint32_t stepCopy = min(length, copySize);
		...
		uint32_t tBytes = min(bytesRead,bytesWritten);
		if( tBytes == 0 )
...
uint32_t __fastcall streamEx_dynamicMemoryRange_readData(void *object, void *buffer, uint32_t len)
{
	...
	uint32_t bytesToRead = min(len, memoryRangeObj->bufferSize - memoryRangeObj->bufferPosition);
