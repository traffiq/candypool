...
void GeneratePrimeTable()
{
    ...
    fprintf(stdout, "GeneratePrimeTable() : setting nSieveExtensions = %u, nSievePercentage = %u, nSieveSize = %u\n", nSieveExtensions, nSievePercentage, nSieveSize);
    ...
    for (unsigned int n = 2; n < nPrimeTableLimit; n++)
        if (!vfComposite[n])
            vPrimes.push_back(n);
    fprintf(stdout, "GeneratePrimeTable() : prime table [1, %u] generated with %u primes\n", nPrimeTableLimit, (unsigned int) vPrimes.size());
}
...
bool TargetGetNext(unsigned int nBits, int64 nInterval, int64 nTargetSpacing, int64 nActualSpacing, unsigned int& nBitsNext)
{
    ...
    if (fDebug && GetBoolArg("-printtarget"))
        fprintf(stdout, "TargetGetNext() : nActualSpacing=%d nFractionDiff=%016"PRI64x" nFractionDiffNew=%016"PRI64x"\n", (int)nActualSpacing, nFractionalDifficulty, nFractionalDifficultyNew);
...
bool CheckPrimeProofOfWork(uint256 hashBlockHeader, unsigned int nBits, const CBigNum& bnPrimeChainMultiplier, unsigned int& nChainType, unsigned int& nChainLength, const bool bSkipTargetCheck /*= false*/)
{
    // Check target
    if( bSkipTargetCheck == false)
    {
...
bool MineProbablePrimeChain(CBlock& block, mpz_class& mpzFixedMultiplier, bool& fNewBlock, unsigned int& nTriedMultiplier, unsigned int& nProbableChainLength, unsigned int& nTests, unsigned int& nPrimesHit, unsigned int& nChainsHit, mpz_class& mpzHash, unsigned int nPrimorialMultiplier, int64& nSieveGenTime, CBlockIndex* pindexPrev)
{
    ...
    while (nTests < nTestsAtOnce && pindexPrev == pindexBest)
    {
        ...
        if (ProbablePrimeChainTestFast(mpzChainOrigin, testParams))
        {
            ...
            fprintf(stdout, "nTriedMultiplier = %u\n", nTriedMultiplier); // Debugging
            fprintf(stdout, "Probable prime chain found for block=%s!!\n  Target: %s\n  Chain: %s\n", block.GetHash().GetHex().c_str(),
...
#if 0
        if(TargetGetLength(nProbableChainLength) >= 1)
            fprintf(stdout, "Multiplier %u gave a prime\n", nTriedMultiplier);
        else
            fprintf(stdout, "Multiplier %u gave nothing\n", nTriedMultiplier);
#endif
    }
