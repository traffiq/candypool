...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    if (fReindex) {
        ...
        while (true) {
            ...
            if (!file)
                break;
            fprintf(stdout, "Reindexing block file blk%05u.dat...\n", (unsigned int)nFile);
    ...
    if (filesystem::exists(pathBootstrap)) {
        ...
        if (file) {
            ...
            fprintf(stdout, "Importing bootstrap.dat...\n");
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    fprintf(stdout, "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    fprintf(stdout, "Primecoin version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    fprintf(stdout, "Using OpenSSL version %s\n", SSLeay_version(SSLEAY_VERSION));
    if (!fLogTimestamps)
        fprintf(stdout, "Startup time: %s\n", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", GetTime()).c_str());
    fprintf(stdout, "Default data directory %s\n", GetDefaultDataDir().string().c_str());
    fprintf(stdout, "Using data directory %s\n", strDataDir.c_str());
    fprintf(stdout, "Using at most %i connections (%i file descriptors available)\n", nMaxConnections, nFD);
    ...
    if (nScriptCheckThreads) {
        fprintf(stdout, "Using %u threads for script verification\n", nScriptCheckThreads);
    ...
    if (!bitdb.Open(GetDataDir()))
    {
        ...
        try {
            ...
            fprintf(stdout, "Moved old %s to %s. Retrying.\n", pathDatabase.string().c_str(), pathDatabaseBak.string().c_str());
    ...
    if (fRequestShutdown)
    {
        fprintf(stdout, "Shutdown requested. Exiting.\n");
        return false;
    }
    fprintf(stdout, " block index %15"PRI64d"ms\n", GetTimeMillis() - nStart);
    ...
    if (mapArgs.count("-printblock"))
    {
        ...
        if (nFound == 0)
            fprintf(stdout, "No blocks matching %s were found\n", strMatch.c_str());
        return false;
    }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            ...
            fprintf(stdout, "%s", strErrors.str().c_str());
    ...
    if (GetBoolArg("-upgradewallet", fFirstRun))
    {
        ...
        {
            fprintf(stdout, "Performing wallet upgrade to %i\n", FEATURE_LATEST);
            nMaxVersion = CLIENT_VERSION;
            pwalletMain->SetMinVersion(FEATURE_LATEST); // permanently upgrade the wallet immediately
        }
        else
            fprintf(stdout, "Allowing wallet upgrade up to %i\n", nMaxVersion);
    ...
    fprintf(stdout, "%s", strErrors.str().c_str());
    fprintf(stdout, " wallet      %15"PRI64d"ms\n", GetTimeMillis() - nStart);
    ...
    if (pindexBest && pindexBest != pindexRescan)
    {
        ...
        fprintf(stdout, "Rescanning last %i blocks (from block %i)...\n", pindexBest->nHeight - pindexRescan->nHeight, pindexRescan->nHeight);
        nStart = GetTimeMillis();
        pwalletMain->ScanForWalletTransactions(pindexRescan, true);
        fprintf(stdout, " rescan      %15"PRI64d"ms\n", GetTimeMillis() - nStart);
        pwalletMain->SetBestChain(CBlockLocator(pindexBest));
        nWalletDBUpdated++;
    }
...

    {
        CAddrDB adb;
        if (!adb.Read(addrman))
            fprintf(stdout, "Invalid or missing peers.dat; recreating\n");
    }

    fprintf(stdout, "Loaded %i addresses from peers.dat  %"PRI64d"ms\n",
    ...
    //// debug print
    fprintf(stdout, "mapBlockIndex.size() = %"PRIszu"\n",   mapBlockIndex.size());
    fprintf(stdout, "nBestHeight = %d\n",                   nBestHeight);
    fprintf(stdout, "setKeyPool.size() = %"PRIszu"\n",      pwalletMain->setKeyPool.size());
    fprintf(stdout, "mapWallet.size() = %"PRIszu"\n",       pwalletMain->mapWallet.size());
    fprintf(stdout, "mapAddressBook.size() = %"PRIszu"\n",  pwalletMain->mapAddressBook.size());
